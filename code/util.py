...
class SeqData(tud.Dataset):
    ...
    def __getitem__(self,ind):#我们这里设置了，marks是一定会有的。
        ...
        #随机挑选一些ind，对应的其实是一个序列，然后随机挑选一个子序列，查看里面是否有目标子序列中的元素，至少有N个，N为超参数，然后不考虑顺序，顺序由更加精细的深度学习来搞定。
        #我们要挑选k个序列，所以直到k之前是不可以停止的，这就有点烦了说实话，因为一个sequence里面有太多子序列，导致要查找很多次，你自己想一想，就算一个序列100个，然后每一个序列要寻找20个，那么这里也有2000个cases啊，也就是100+2000的长度。所以只可以bs=1了。
        N=20
        count=0
        while count<N:
            seqid=np.random.randint(len(self.eventlen))#
            if seqid!=ind:#说明是可以继续进行下去的。
                if seqid == 0:
                    start = 0
                else:
                    start = self.eventlen[seqid - 1]
                end = self.eventlen[seqid]
                seq = self.seqs[start:end]  # 我明白了，不复存在就不复存在，无非是使用两份代码而已。但是真的可以快很多啊。[1,seqlen]
                mask = self.masks[start:end]
